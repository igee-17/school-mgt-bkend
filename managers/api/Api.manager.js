...
module.exports = class ApiHandler {
    ...
    async mw(req, res, next){
        ...
        /** validate module */
        if(!moduleMatrix) return this.managers.responseDispatcher.dispatch(res, {ok: false, code: 404, message: `module '${moduleName}' not found`});

        /** validate method + function — distinguish wrong-method (405) from not-found (404) */
        if(!moduleMatrix[method] || !moduleMatrix[method].includes(fnName)){
            const correctMethod = Object.keys(moduleMatrix).find(m => moduleMatrix[m].includes(fnName));
            if(correctMethod){
                return this.managers.responseDispatcher.dispatch(res, {ok: false, code: 405, message: `'${fnName}' requires ${correctMethod.toUpperCase()}, not ${method.toUpperCase()}`});
            }
            return this.managers.responseDispatcher.dispatch(res, {ok: false, code: 404, message: `'${fnName}' not found in module '${moduleName}'`});
        }
